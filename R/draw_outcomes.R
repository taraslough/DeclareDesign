...
draw_potential_outcomes <- function(data, potential_outcomes, condition_names = NULL) {
  ...
  has_treatment_variables <- all(sapply(potential_outcomes, function(x) !is.null(x$treatment_variable))) == TRUE
  create_columns <- has_condition_names & has_treatment_variables
  
  if(has_condition_names & !has_treatment_variables){
    stop("Please provide the name of the treatment variable to the treatment_variable argument in declare_potential_outcomes if you provide condition_names.")
  }
  
  if(sapply(potential_outcomes, function(x) !is.null(x$treatment_variable)) != length(potential_outcomes)){
    stop("If you provide a treatment_variable for any of the potential_outcomes, you must provide it for all of them.")
  }
  ...
  if(create_columns == TRUE) {
    for(i in 1:length(potential_outcomes)){
      for(j in 1:length(condition_names[[i]])){
        data_internal <- data
        data_internal[potential_outcomes[[i]]$treatment_variable] <- condition_names[[i]][j]
