---
title: 'Simple analysis with experimentr'
author: 'Blair, Cooper, Coppock, and Humphreys'
output:
  html_document:
    highlight: tango
    number_sections: yes
...
Your four main steps in describing a design are to describe:

1. the **environment**: how many cases are there? what sort of data do you have on these, etc
...
```{r}
rm(list=ls())
# devtools::install_github(repo = "egap/experimentr", auth_token = "c5a7798ff2bc621f161f6e6a8155a4ecc1985045")
library(experimentr)
```
...
```{r}
my.sample.frame <- declare_sample_frame(
  individuals = list(
...
```{r}
my.design <- declare_design(potential_outcomes = my.potential.outcomes, 
...
```{r}
my.mock.data <- make_data(potential_outcomes = my.potential.outcomes, sample_frame = my.sample.frame, 
                  blocks = declare_blocks(blocks = "type", block_name = "my_blocks", block_count = 2))
...
## Diagnostics
Now things get more interesting.  Let's see how good this design is. Note that given the potential outcomes is it indeed the case that the true treatment effect is +2 for one group and -1 for another. With even sized groups that means an average treatment effect of +1.


```{r}
my.diagnostics         <- simulate_experiment(sims = 1000, analysis = list(my.lsdv.analysis), 
                          design = my.design, 
                          sample_frame = my.sample.frame, 
...
* Second the "simulation error" is quite large; here this is the standard deviation of the SATE: or the difference between the SATE and the PATE given a design in which we draw just 20 units from a larger population. 
