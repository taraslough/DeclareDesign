...
my.potential.outcomes     <-  declare_potential_outcomes(
    ...
    formula = Y ~ 0*Z0*(type=="A") + 2*Z0*(type=="B") + 3*Z1*(type=="A") + 1*Z1*(type=="B")
    )  
```
...
my.mock.data <- make_data(potential_outcomes = my.potential.outcomes, sample = my.sample.frame, 
                  blocks = declare_blocks(blocks = "type", block_name = "my_blocks", block_count = 2))
my.mock.data$Z <- assign_treatment_indicator(design = my.design, data = my.mock.data)
my.mock.data$Y <- reveal_outcome(outcome = "Y", treatment_assignment = "Z", data = my.mock.data)
head(my.mock.data)
```
...
```{r}
my.diagnostics         <- diagnose_design(sims = 1000, analysis = list(my.lsdv.analysis), 
                          design = my.design, 
