...
test_that("test simple interference model", {
  ...
  potential_outcomes <- declare_potential_outcomes(formula = Y ~ 5 + .5*Z + .15*I + noise,
                                                   condition_names = c(0, 1),
                                                   assignment_variable_name = "Z")
  
  adj <- rgnm(n = 1, nv = 100, m = 2000)
  interference <- declare_interference(formula = I ~ .2 * adj %*% Z, 
  ...
  pop_draw <- draw_population(population = population, potential_outcomes = potential_outcomes,
                              interference = interference, condition_names = c(0, 1))
