...
To make user-defined covariates, we can provide `declare_sample_frame()` 
...
sample_frame_1 <- declare_sample_frame(
  declare_variable(),
  N = N
)
...
sample_frame_2 <- declare_sample_frame(
...
sample_frame_3 <- declare_sample_frame(
  # Using functions
...
sample_frame_4 <- declare_sample_frame(
...
sample_frame_5 <- declare_sample_frame(
  # Declare the raw variables 
...
sample_frame_6 <- declare_sample_frame(
  firm = list(
...
In order to create levels, the user simply provides `declare_sample_frame()`
...
sample_frame_7 <- declare_sample_frame(
 student = list(
...
sample_frame_8 <- declare_sample_frame(
 student = list(
   math_score = declare_variable(),
   lang_score = declare_variable()
 ),
...
simple_frame <- declare_sample_frame(N = 10)
...
voter_sample <- declare_sample_frame(
  N_per_level = c(10,2),
...
simple_frame_2 <- declare_sample_frame(
...
among the variable defined at the cluster level. If we thought the standard 
deviation in the outcome was, for example, 10, then we could create scenarios
in which the ICC was low, say .1, and high, say .6, by setting the 
cluster-level standard deviation to $\sigma_j =$ 
$\sqrt{\frac{\sigma_i^2(1 - \text{ICC})}{\text{ICC}}} =$ $\sqrt{10^2(1 - .1)/.1}$ or 
$\sqrt{10^2(1 - .6)/.6}$ 

```{r}
# Set the standard deviation of the outcome
sigma_i <- 10
# Set the two ICC you would like to compare
ICC_1 <- .1
ICC_2 <- .6
# Set the two cluster shocks
sigma_j_1 <- sqrt((sigma_i^2 * (1 - ICC_1))/ICC_1)
sigma_j_2 <- sqrt((sigma_i^2 * (1 - ICC_2))/ICC_2)

sigma_i^2 / (sigma_j_1^2 + sigma_i^2)
sigma_i^2 / (sigma_j_2^2 + sigma_i^2)

# Make a clustered sample frames
cluster_frame <- declare_sample_frame(
  unit = list(
    baseline = declare_variable(normal_mean = 0,normal_sd = sigma_i)
  ),
  # The shocks that create the ICC go here
  cluster = list(
    cluster_shock_1 = declare_variable(normal_mean = 0,normal_sd = sigma_j_1),
    cluster_shock_2 = declare_variable(normal_mean = 0,normal_sd = sigma_j_2)
  ),
...
cluster_PO_1 <- declare_potential_outcomes(
  condition_names = c("Z0","Z1"),
  outcome_formula = Y ~ baseline + cluster_shock_1 + Z0*0 + Z1*.2
)
cluster_PO_2 <- declare_potential_outcomes(
  condition_names = c("Z0","Z1"),
  outcome_formula = Y ~ baseline + cluster_shock_2 + Z0*0 + Z1*.2
)
...
### SOMETHING WRONG HERE, ICC should be .1:
sigmasq_i_1 <- var(data_1$Y_Z0)
sigmasq_j_1 <- var(tapply(data_1$Y_Z0,data_1$cluster_id,mean))
sigmasq_i_1/(sigmasq_i_1+sigmasq_j_1)    

# This looks *better*, ICC should be .6:
sigmasq_i_2 <- var(data_2$Y_Z0)
sigmasq_j_2 <- var(tapply(data_2$Y_Z0,data_2$cluster_id,mean))
sigmasq_i_2/(sigmasq_i_2+sigmasq_j_2)    
```
