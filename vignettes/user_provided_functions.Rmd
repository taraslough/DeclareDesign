...
potential_outcomes     <-  declare_potential_outcomes(
  ...
  outcome_formula = Y ~ rbinom(500, size = 1, prob = pnorm(Y0*Z0 + Y1*Z1))
)
```
...
population_custom <- declare_population(custom_population_function = make_clustered_data_experimentr, 
...
analysis_custom      <- declare_analysis(formula = Y ~ Z_star, treatment_variable = "Z_star", 
                                         estimator = probit_regression, 
                                         quantity_of_interest = probit_qoi_temp,
                                         estimand = difference_in_means, estimand_formula = Y ~ Z_star)
```
...
```{r}
simulations <- diagnose(population = population_custom, design = design_custom, analysis = analysis_custom)
summary(simulations)
```
...
```{r}
mock <- draw_population(population = population_custom)
mock <- reveal_design(data = mock, design = design_custom)
kable(head(mock), digits = 3)
```

```{r}
kable(get_estimates(analysis = analysis_custom, data = mock))
```
