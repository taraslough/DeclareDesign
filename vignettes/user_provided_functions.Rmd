...
potential_outcomes     <-  declare_potential_outcomes(
  ...
  outcome_formula = Y ~ logistic(.01 + 0*Z0 + .2*Z1 + .1*income)
)
```
...
```{r}
regression_custom <- function(data){
  reg <- summary(lm(Y ~ Z, data = data))
  coef <- reg$coefficients[2, 1]
...
analysis      <- declare_analysis(formula = Y ~ Z, treatment_variable = "Z", estimator = regression_custom)
```
...
pre <- pre_register(design = design, sample = sample, 
                    ...
                    author = c("Ronald A. Fisher"), 
                    abstract = "Description of the lady tasting tea experiment",
                    temp_dir = TRUE, make_output = FALSE, open_output = FALSE)
```
