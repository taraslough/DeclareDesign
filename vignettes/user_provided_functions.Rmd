...
make_clustered_data <- function(J = 10, n = 100, treatment_effect = .25, ICC = .1){
  ...
  fake_data$Y_Z0 <- rnorm(n * J,0,sd = (1 + treatment_effect) ^ 2 * sqrt(1 - ICC)) + Y0_j[fake_data$j]
  fake_data$Y_Z1 <- with(fake_data,mean(Y_Z0) + treatment_effect + (Y_Z0 - mean(Y_Z0)) * (2 / 3))
  fake_data$Z  <- ifelse(fake_data$j %in% sample(1:J,J / 2) == TRUE, 1, 0)
  fake_data$Y  <- with(fake_data, Z * Y_Z1 + (1 - Z) * Y_Z1)
  return(fake_data)
}
...
potential_outcomes     <-  declare_potential_outcomes(
  ...
  outcome_formula = Y_star ~ rbinom(500, size = 1, prob = pnorm(Y_Z0*Z0 + Y_Z1*Z1))
)
```
