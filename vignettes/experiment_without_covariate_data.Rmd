...
library(xtable)
set.seed(20)
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, error = TRUE, message = FALSE, eval = TRUE, tidy = TRUE, tidy.opts = list(width.cutoff = 50),
...
Before finalizing the design, we conduct a power analysis to determine whether 500 units and 10 clusters (villages) are sufficient. To do this, we use the get_diagnostics function.

```{r}
simulations <- get_diagnostics(potential_outcomes = potential_outcomes, 
...
comparisons <- compare_experiments(N = c(100, 500, 750, 1000, 1250, 1500),
                                   ...
                                   analysis = analysis, 
                                   sims = 100)
print(xtable(summary(comparisons), digits = 3))
```
...
```{r}
sample_frame_block_cluster <- declare_sample_frame(
  individuals = list(
    income = declare_variable()),
  villages = list(
...
analysis_block_cluster <- declare_analysis(formula = Y ~ Z, treatment_variable = "Z", estimator = difference_in_means)
```
...
```{r}
simulations_block_cluster <- get_diagnostics(potential_outcomes = potential_outcomes_block_cluster, 
                                   sample_frame = sample_frame_block_cluster, 
...
comparisons_block_cluster  <- compare_experiments(
  ...
  sample_frame = list(sample_frame, sample_frame_block_cluster), 
...
```{r}
mock <- make_data(sample_frame = sample_frame_block_cluster, potential_outcomes = potential_outcomes_block_cluster, 
                  clusters = clusters, blocks = blocks)
mock$Z <- assign_treatment(design = design_block_cluster, data = mock)
mock$Y <- observed_outcome(outcome = "Y", treatment_assignment = "Z", data = mock)
```
...
```{r, eval = FALSE}
pre <- pre_register(design = design_block_cluster, sample_frame = sample_frame_block_cluster, 
