...
```{r}
xtable(get_estimates(analysis = analysis, data = mock))
```
...
```{r}
##draft_paper_from_pre_register(pre, data = mock, temp_dir = TRUE)
```
...
```{r}
design_block_cluster <- declare_design(potential_outcomes = potential_outcomes, 
...
```{r}
pre <- pre_register(design = design, sample_frame = sample_frame, blocks = blocks, clusters = clusters,
                    potential_outcomes = potential_outcomes, analysis = analysis, 
                    title = "Lady Tasting Tea", 
                    author = c("Ronald A. Fisher"), 
                    abstract = "Description of the lady tasting tea experiment",
...
##draft_paper_from_pre_register(pre, data = mock)
```
